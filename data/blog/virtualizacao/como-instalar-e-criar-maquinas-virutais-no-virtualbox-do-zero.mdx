...
- [Instalando o VirtualBox no Windows](#instalando-o-virtualbox-no-windows)
  - [Pré-requisitos para o VirtualBox no Windows](#pré-requisitos-para-o-virtualbox-no-windows)
    ...
    - [3. Ativando o suporte à virtualização no Windows](#3-ativando-o-suporte-à-virtualização-no-windows)
...
### 3. Ativando o suporte à virtualização no Windows

No Windows 10/11 é necessário ativar a **Windows Hypervisor Platform (Plataforma de Hipervisor do Windows)**. Essa plataforma apenas expõe APIs que permitem ao VirtualBox usar as instruções de virtualização do processador.

**Ativando via interface gráfica:**

- Pesquise por _“Ativar ou desativar recursos do Windows”_.
- Marque a opção **Plataforma de Hipervisor do Windows (Windows Hypervisor Platform)**.
- Reinicie o computador.

**Ativando via PowerShell (como Administrador):**

```powershell
Enable-WindowsOptionalFeature -Online -FeatureName HypervisorPlatform -All
```

> Outro detalhe importante é que você deve ativar o Intel VT-x ou AMD-V nas opções do processador na configuração da sua placa-mãe. Sem isso, a virtualização não funcionará corretamente, mesmo com as opções do Windows habilitadas.
