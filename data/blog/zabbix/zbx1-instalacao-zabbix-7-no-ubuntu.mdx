...
- [Instalação Centralizada](#instalação-centralizada)
  - [Atualização do sistema e instalação do banco de dados](#atualização-do-sistema-e-instalação-do-banco-de-dados)
  - [Instalação e configuração da aplicação Zabbix](#instalação-e-configuração-da-aplicação-zabbix)
  - [Últimas configurações e primeiro acesso](#últimas-configurações-e-primeiro-acesso)
  - [Script de Instalação](#script-de-instalação)
...
(/var/lib/\<mysql,postgres,etc\>) em partições separadas. Inclusive, se utilizar múltiplos _tier_ de disco no seu ambiente, utilize
o com maior número de _IOPS_ para a partição do banco de dados.
...
Lembre-se de alterar os valores de _ZABBIX_DB_, _ZABBIX_DB_USER_ e _ZABBIX_DB_PASSWORD_ para os valores reais de sua instalação.
...
Para não ser ncessário utilizar um editor de texto com oo Nano ou Vim, deixo abaixo os comadnos utilizando o _sed_ para alterar as linhas
...
O usuário padrão é _Admin_ e a senha é _zabbix_.
