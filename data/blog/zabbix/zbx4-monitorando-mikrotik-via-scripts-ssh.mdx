...
- [Explicação geral do funcionamento dos scripts](#explicação-geral-do-funcionamento-dos-scripts)
- [Quantidade de Leases DHCP por Interface](#quantidade-de-leases-dhcp-por-interface)
  - [Configuração do item no Zabbix](#configuração-do-item-no-zabbix)
- [Quantidade de sessões PPPoE por Interface](#quantidade-de-sessões-pppoe-por-interface)
  - [Configuração do item no Zabbix](#configuração-do-item-no-zabbix-1)
- [Estado do vizinho OSPF](#estado-do-vizinho-ospf)
  - [Configuração do item no Zabbix](#configuração-do-item-no-zabbix-2)
...
O script deve ser salvo em _System -> Scripts_ e conseguimos rodá-lo pela CLI do Mikrotik por meio do comando `system script run nome_do_script`.
